...
with open('firms_suppl_info.json','r') as f:
    firms_suppl_info=json.load(f)
    for k in firms_suppl_info:
        if k in to_grade:
            to_grade[k]=firms_suppl_info[k]
...
def force_classify(to_grade):
    ...
    global class_gpt_2
    global class_gpt
    forced={'synd agro':UniqueList(),'privé':UniqueList(),'syndicat':UniqueList(),'public':UniqueList(),'collectivite':UniqueList(),'mutuelle':UniqueList(),'match':UniqueList()}
    counter=0
    unmapped_act=set()
    for k in to_grade:
        counter+=1
        ...
        try:
            ...
            try:
                ...
                try:#on cherche les activités des "établissements"
        ...
        if contient_simpl(k+' '+str(dict_k),['SYNDICATS EXPLOITANT AGRI','FRSEA','FNSEA','FDSEA','chambre d agric','chambre agric',"chambre d'agric",'jeunes agr',"Fédération Nationale des Syndicats d'Exploitants Agricoles","Fédération Départementale des Syndicats d'Exploitants Agricoles","Fed nat synd expl","fed dep synd expl",'DEP SYND EXPLOITANT AGRI',"FEDE NAT SYNDIC EXPL AGRICOLE",'syndicat agricole','syndicat des agriculteurs','syndicat agricole','syndicat des exploitants agricoles','syndicat des eleveurs',"CHAMBRE DEP D'AGRI","CHAMBRE DEP D AGRI","CHAMBRE DEPartementale D'AGRIC","CHAMBRE DEPartementale D AGRICULTURE","SYNDIC EXPLOIT AGRI"]):
            ...
            class_gpt_2[k]={'classes':{"syndicat":50,'prive':50},'explication':'syndicat agricole','manuel':True}
        elif begin_with(k,['commune','region ','agglomeration','departement ','metropole','canton ','ville de ']):
            print('collectivite',k)
            forced['collectivite'].add(k)
            class_gpt_2[k]={'classes':{"collectivite":100},'manuel':True,'explication':'nom' }
            ...
            class_gpt_2[k]={'classes':{"syndicat":100} ,'manuel':True,'explication':'syndicat mixte' }
        elif contient_simpl(k,['Syndicat','synd ','syndical']):
            print('synd',k)
            forced['syndicat'].add(k)
            synd=class_gpt[k]['classes']['syndicat'] if 'syndicate' in class_gpt[k]['classes'] else 0
            priv=class_gpt[k]['classes']['prive'] if 'prive' in class_gpt[k]['classes'] else 0
            
            if synd+priv>0:#on garde les proportions du vieux class_gpt
                new_synd=int(100*synd/(synd+priv))
                class_gpt_2[k]={'classes':{"syndicat":new_synd,'prive':100-new_synd} ,'manuel':True,'explication':'nom et class_gpt_old' }
            else:
                class_gpt_2[k]={'classes':{"syndicat":50,'prive':50} ,'manuel':True,'explication':'nom' }
            ...
            print('public',k)
            forced['public'].add(k)
            coll=class_gpt[k]['classes']['collectivite'] if 'collectivite' in class_gpt[k]['classes'] else 0
            pub=class_gpt[k]['classes']['public'] if 'public' in class_gpt[k]['classes'] else 0
            if coll+pub>0:
                new_coll=int(100*coll/(coll+pub))
                class_gpt_2[k]={'classes':{"public":100-new_coll,"collectivite":new_coll},'manuel':True,'explication':'nom ou infos dans fichier auxiliaire et vieux class_gpt'}
            else:
                class_gpt_2[k]={'classes':{"public":100},'manuel':True,'explication':'nom ou infos dans fichier auxiliaire'}
        ...
        #ne pas modifier l'ordre des elif
            ...
            if synd+priv>0:#on garde les proportions du vieux class_gpt
                ...
                class_gpt_2[k]={'classes':{"prive":100} ,'manuel':True,'explication':'nom' }
        else:
            print('unfound')
            if 'activite_principale' in dict_k:
                unmapped_act.add(dict_k['activite_principale'])
        class_gpt_2[k]['info insee']=dict_k

    print('forced',sum(len(forced[k]) for k in forced) )
    input('?')
    matches=set()
    ungraded=[k for k in class_gpt_2 if 'classes' not in class_gpt_2[k]]
    counter=0
    for k in ungraded:
        counter+=1
        print(counter,'/',len(ungraded))
        print(k)
        dict_k=to_grade[k]
        name_keys=['nom_complet','nom_raison_sociale']
        name_keys_hatvp=['denomination','nomUsage','sigleHatvp']
        other_names=[dict_k[n] for n in name_keys if n in dict_k] + [dict_k['firme'][n] for n in name_keys_hatvp if 'firme' in dict_k and n in dict_k['firme']]
        print('other names',other_names)
        best_match=find_best_match([k]+other_names,{k:class_gpt_2[k] for k in class_gpt_2 if k not in ungraded})
        if (best_match['best_score']>85 and len(k)>5) or best_match['best_score']>95:
            print('best match',best_match['best_match']['ressemblance avec '])
            forced['match'].add(k)
            ...
            class_gpt_2[k]={'classes':class_gpt_2[best_match['best_match']['ressemblance avec ']]['classes'],'explication':'ressemblance nom avec '+best_match['best_match']['ressemblance avec '],'manuel':True}
        else:
            class_gpt_2[k]=class_gpt[k]
            print('unfound again')
            unfounds[k]=dict_k
        class_gpt_2[k]['info insee']=dict_k
